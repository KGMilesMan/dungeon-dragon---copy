...
let Gap_image: Image = null
let bottom_image: Image = null
let Whole = 0
let Top_whole = 0
...
game.onUpdateInterval(1500, function () {
    Top_whole = randint(0, 2)
    if (Whole == 0) {
        Whole = sprites.dungeon.floorDark1
        bottom_image = sprites.dungeon.floorDark1
    } else if (Whole == 1) {
        Top_whole = sprites.dungeon.floorDark1
        bottom_image = sprites.dungeon.floorDark1
    } else if (Whole == 2) {
        Top_whole = sprites.dungeon.floorDark1
        bottom_image = sprites.dungeon.floorDark1
    ...
    Gap_image = image.create(2, scene.screenHeight())
    Gap_image.fill(0)
    Gap_sprite = sprites.create(Gap_image, SpriteKind.Hole)
    mySprite.setFlag(SpriteFlag.AutoDestroy, true)
    mySprite.setFlag(SpriteFlag.Invisible, true)
    mySprite.left = scene.screenWidth()
    Gap_image.vx = -45
    Gap_sprite = sprites.createProjectileFromSide(Top_whole, -45, 0)
    projectile.top = 0
    projectile = sprites.createProjectileFromSide(bottom_image, -45, 0)
    projectile.x = scene.screenHeight()
})
